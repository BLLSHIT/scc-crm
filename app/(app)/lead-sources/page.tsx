...
import { getLeadSources } from '@/lib/db/lead-sources'
...
export default async function LeadSourcesPage() {
  ...
  try {
    ...
    rows = await getLeadSources()
  } catch (err) {
    if (isFrameworkError(err)) throw err
    return <ErrorView where="Lead-Quellen laden" err={err} />
  }

  return (
    <div className="flex-1 overflow-auto">
      <Header
        title={`Lead-Quellen (${rows.length})`}
        profile={profile}
        actions={
          <Link href="/lead-sources/new" className={buttonVariants({ size: 'sm' })}>
      ...
      <main className="p-6">
        <p className="text-sm text-slate-500 mb-4">
          Wird im Kontakt-Formular als Dropdown angeboten („Quelle").
        </p>
        <div className="bg-white rounded-xl border overflow-hidden max-w-3xl">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {rows.length === 0 && (
                <tr><td colSpan={3} className="px-4 py-12 text-center text-slate-400">Keine Lead-Quellen.</td></tr>
              )}
              {rows.map((i) => (
                <tr key={i.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3">
                    <Link href={`/lead-sources/${i.id}/edit`} className="font-medium text-slate-900 hover:text-blue-600">
                      {i.name}
                    </Link>
                  </td>
